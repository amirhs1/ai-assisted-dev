...
1. **Disclose assistance in the PR description.** Tool plus model identifier
   and role/extent (for example, `full implementation`, `partial implementation`,
   `review`). If the exact model was not recorded, say `not recorded` rather
   than guessing. One sentence is enough. Commit trailers per `AI-POLICY.md`
   `[PROV-1]` are welcome but not required from outside contributors.
...
4. **Unknown provenance is a verification question, not an accusation.**
   Where AI provenance is unknown, or where the change cannot be readily
   validated from its tests, references, and explanation, the maintainer may
   apply Supervised (T2)-level diff-reading rigor and treat test assertions as
   unverified until independently assessed. You may be asked to restate what the
   change does and why.
...
1. **Disclose assistance in the PR description** — tool plus model identifier
   ...
   rather than guessing. One sentence is enough; commit trailers are welcome but
   not required from outside contributors.
2. **No circular ground truth.** A model may draft a test or run a check, but
   ...
   defensible *property* check. Any citation a model gave you is unverified until
   you have opened it and checked the claim it supports.
3. **Do not weaken or delete a test to make a suite pass.** A diff changing an
   ...
   description.
4. **Unknown provenance is a verification question.** Where AI provenance
   is unknown, or the change cannot be readily validated from tests, references,
